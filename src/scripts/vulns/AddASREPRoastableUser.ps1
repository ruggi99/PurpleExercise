...
for ($i=0; $i -lt $limit; $i=$i+1 ) {
    ...
    $password = Get-Random -InputObject $BadPasswords;
    $SamAccountName, $notused = AddADUser -password $password
    
    Invoke-Command -ComputerName $config.domain.dcip -Credential $admin -ScriptBlock {
        Set-ADAccountControl -Identity $using:SamAccountName -DoesNotRequirePreAuth 1
    }
}